...
test('keystroke outdents current task', async () => {
    const user = userEvent.setup()
    const taskData: TaskV2SetV2 = {
        ...defaultTaskV2SetV2Data,
        0: {
...
test('keystroke initiates editing for current task', async () => {
    const user = userEvent.setup()
    const taskData: TaskV2SetV2 = {
        ...defaultTaskV2SetV2Data,
        0: {
...
test('keystroke initiates editing for newly created child task', async () => {
    const user = userEvent.setup()
    const taskData: TaskV2SetV2 = {
        ...defaultTaskV2SetV2Data,
        0: {
    ...
    await act(async () => {
        await user.keyboard('{Shift>}+{/Shift}four{Escape}')
    })
    ...
    expect(screen.queryByText('three')).toHaveClass('focus-task')
    expect(screen.queryByText('three')).not.toHaveClass('focus-current-task')
    expect(screen.queryByText('four')).toHaveClass('focus-task')
    expect(screen.queryByText('four')).toHaveClass('focus-current-task')
})
